...
setup_fmt = 'VERSION = \'{}\''
readme_fmt = 'https://github.com/colbylwilliams/az-util/releases/latest/download/util-{}-py2.py3-none-any.whl'
history_fmt = '{}\n++++++\n{}\n\n{}'
...
with open(Path(Path.cwd() / 'util') / 'setup.py', 'w') as f:
    f.write(setup)


print('..updating HISTORY.rst')

with open(Path(Path.cwd() / 'util') / 'HISTORY.rst', 'r') as f:
    history = f.read()

history = history.replace(v.public, history_fmt.format(n.public, notes, v.public))

with open(Path(Path.cwd() / 'util') / 'HISTORY.rst', 'w') as f:
    f.write(history)
